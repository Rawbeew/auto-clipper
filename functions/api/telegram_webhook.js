...
export async function onRequestPost(context) {
  try {
    ...
    // A. Character Locking Matching (/character <name>)
    if (rawText.startsWith("/character")) {
      const charChoice = rawText.replace("/character", "").trim() || "tax_advisor";
      await replyTelegram(`🔒 *Locking Mascot Character to:* \`${charChoice}\`...\n\n_Updating character configuration in permanent pipeline memory!_`);
    ...
    // B. Trend Research matching (/trending, /ideas, "fresh viral ideas")
    ...
    // C. Longform Documentary matching (/longform)
    ...
    // D. Default: Video Generation Prompt (/make <topic>, or any topic/link text)
    const cleanPrompt = rawText.replace("/make", "").trim() || rawText;
