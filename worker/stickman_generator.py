...
class StickmanGenerator:
    ...
    Supports True Crime Noir, Slate Minimal, and Neon styles with OpenAI Onyx deep voice narration.
    """
    ...
    def generate_script(self, topic: str) -> dict:
        prompt = f"""
You are a viral YouTube Shorts, TikTok, and True Crime stickman animator (like Casually Explained / Cold Case Detective).
...
2. "scenes": A list of 4 to 6 scenes. Each scene must have:
   ...
   - "pose": Stickman pose choice. Choose from: ["detective_magnifying", "thinking", "pointing", "mind_blown", "confused", "running", "celebrating"]
   - "text_overlay": Big punchy headline text displayed on screen (3-6 words max)
   - "prop": Optional prop icon ["magnifying_glass", "lightbulb", "money", "fire", "question_mark", "none"]

Format: JSON strictly.
"""
        res = self.ai_provider.generate_json(prompt, system_prompt="You are an expert viral True Crime & mystery stickman scriptwriter. Respond strictly in valid JSON.")
        if res:
            return res
            ...
            "scenes": [
                {
                    "scene_num": 1,
                    "duration": 5.0,
                    "narration": f"In the winter of 1992, an unsolved case shook the city. Here is the mystery of {topic}.",
                    "pose": "detective_magnifying",
                    "text_overlay": f"THE MYSTERY OF {topic.upper()[:18]}",
                    "prop": "magnifying_glass"
                },
                {
                    "scene_num": 2,
                    "duration": 6.0,
                    "narration": "Investigators found zero forced entry, but one critical clue was left behind.",
                    "pose": "thinking",
                    "text_overlay": "THE HIDDEN CLUE",
                    "prop": "question_mark"
                },
                {
                    "scene_num": 3,
                    "duration": 6.0,
                    "narration": "When forensic scientists analyzed the evidence, the truth was mind blowing.",
                    "pose": "mind_blown",
                    "text_overlay": "SHOCKING TWIST!",
                    "prop": "fire"
                },
                {
                    "scene_num": 4,
                    "duration": 5.0,
                    "narration": "To this day, the case remains officially unsolved.",
                    "pose": "confused",
                    "text_overlay": "CASE UNSOLVED",
                    "prop": "none"
                }
            ]
        }
    ...
    def draw_stickman_frame(self, pose: str, text_overlay: str, prop: str, frame_num: int, width=1080, height=1920) -> Image.Image:
        bg_color = (2, 6, 23) if pose == "detective_magnifying" else (15, 23, 42)
        ...
        accent_color = (225, 29, 72) if pose == "detective_magnifying" else (99, 102, 241)
        head_r = 75
        body_len = 220
        line_w = 16
        ...
        if pose == "detective_magnifying":
            ...
            draw.polygon([
                ...
                (cx - 60, head_cy - head_r - 50)
            ], fill=accent_color, outline=stroke_color)
            draw.ellipse([cx - 30, head_cy - 10, cx - 10, head_cy + 5], fill=stroke_color)
            draw.ellipse([cx + 10, head_cy - 10, cx + 30, head_cy + 5], fill=stroke_color)
        elif pose == "confused":
            ...
            for angle in range(0, 360, 45):
                ...
                draw.line([x1, y1, x2, y2], fill=(225, 29, 72), width=10)
        else:
        ...
        # 3. Arms & Legs
        if pose == "detective_magnifying" or prop == "magnifying_glass":
            ...
            draw.ellipse([cx + 80, shoulder_y - 80, cx + 160, shoulder_y], outline=(225, 29, 72), width=10)
            draw.line([cx + 120, shoulder_y, cx + 160, shoulder_y + 40], fill=stroke_color, width=12)
        elif pose == "pointing":
        ...
        # 4. Text Overlay Banner
        if text_overlay:
            padding_y = 180
            clean_text = text_overlay.encode('ascii', 'ignore').decode('ascii')
            try:
                font_title = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 54)
            except:
                font_title = ImageFont.load_default()

            bbox = draw.textbbox((0, 0), clean_text, font=font_title)
            text_w = bbox[2] - bbox[0]
            banner_bg = (225, 29, 72) if pose == "detective_magnifying" else (2, 6, 23)
            draw.rectangle([0, padding_y - 20, width, padding_y + 80], fill=banner_bg)
            draw.text(((width - text_w) // 2, padding_y), clean_text, fill=(255, 255, 255), font=font_title)
    ...
    def create_stickman_video(self, topic: str) -> str:
        ...
        print(f"✅ Stickman Video successfully generated: {final_output_mp4}")
        return final_output_mp4

if __name__ == "__main__":
    generator = StickmanGenerator()
    generator.create_stickman_video("The $100M Antwerp Diamond Heist")
